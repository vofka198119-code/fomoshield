...
- **Публичный репозиторий релизов** `vofka198119-code/fomoshield-releases` — зеркало
  бинарников: джоба `publish` (единственный релизный шаг) заливает туда `ScanCo.*`
...
- Логи запусков: читаются **только через GitHub CLI** (`gh run view <run-id> --log`).
  ...
  grep (не коммитить).
- **После push — подтверждать, что CI собрался** (tests → builds → publish), обычно
  ~10 минут. ВАЖНО: ассистент не может «поставить таймер на 10 минут» и проверить
  сам по себе — он действует только когда его вызывают. Рабочие варианты:
  1) сразу после push запустить **ожидание с дедлайном** (~15 мин):
     `gh run list --workflow release.yml --limit 1` → `gh run view <id>` (или цикл
     опроса `gh api .../runs/<id>`) и доложить conclusion;
  2) если запуск ещё идёт, а пользователь вернулся раньше — при следующем обращении
     проверить статус и сообщить результат (зелёный/красный; какие платформы
     пропущены — macOS/iPhone skippable на ветках).
