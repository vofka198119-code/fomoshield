// ---------------------------------------------------------------------------
// MSFT Stress Test Simulation — Buy MSFT at $376, hold 1 month accelerated
// ---------------------------------------------------------------------------
...
void main() {
  test('MSFT 1-month stress test — buy & hold', () async {
    TestWidgetsFlutterBinding.ensureInitialized();
    SharedPreferences.setMockInitialValues({});

    final notifier = StressTestNotifier(userId: 'msft_test');

    // --- Setup ---
    const double msftEntryPrice = 376.0;
    ...
    final bought = await notifier.buyAssetSetup(id, 'MSFT', investment, msftEntryPrice);
    ...
    print('\n========== MSFT STRESS TEST — 1 MONTH ==========');
    print('  Entry:  MSFT \$${msftEntryPrice.toStringAsFixed(2)}');
    ...
    for (int i = 0; i < totalEpochs; i++) {
      ...
      final s = notifier.getSession(id)!;
      priceHistory.add(s.currentPrices['MSFT'] ?? msftEntryPrice);
    ...
    final finalPrice = session.currentPrices['MSFT'] ?? msftEntryPrice;
    ...
    print('\n---------- PRICE BEHAVIOR ----------');
    double maxPrice = msftEntryPrice, minPrice = msftEntryPrice;
    int corrections = 0;
    ...
    print('  Volatility:  \$${(maxPrice - minPrice).toStringAsFixed(2)} ' +
          '(${((maxPrice - minPrice) / msftEntryPrice * 100).toStringAsFixed(1)}%)');
