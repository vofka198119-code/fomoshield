      ...
      children: [
        Material(
          ...
          child: InkWell(
            onTap: onTap,
            child: Container(
              // A palette is only supplied once the caller has opted into
              // theming (see class doc); without one this keeps the exact
              // original hardcoded decoration so every existing call site
              // that doesn't pass palette stays byte-for-byte unchanged.
              decoration: (showDivider && palette == null)
                  ? const BoxDecoration(
                      border: Border(
                        bottom: BorderSide(
        ...
        if (showDivider && palette != null) themedRowDivider(palette!),
      ],
    );
  }
}
