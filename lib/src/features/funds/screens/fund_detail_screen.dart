...
import '../widgets/fund_investments_stub_card.dart';
...
// via fundDetailProvider. Buy/Sell (Phase 2) and hiring (Phase 3) aren't
// built yet — their cards render as stubs until then.
// ---------------------------------------------------------------------------
          ...
          data: (fund) => RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                // padding already (see price_header.dart) — no extra wrap
                // here, or it'd double up like every other card below.
                FundPriceHeader(fund: fund, palette: palette),
                const SizedBox(height: 16),
                _padded(FundNavChart(fund: fund, palette: palette)),
                const SizedBox(height: 16),
                _padded(FundInvestmentsStubCard(palette: palette)),
                const SizedBox(height: 16),
                _padded(FundKeyMetricsCard(fund: fund, palette: palette)),
                const SizedBox(height: 16),
                _padded(FundSectorAllocationCard(fund: fund, palette: palette)),
                const SizedBox(height: 16),
