...
// ---------------------------------------------------------------------------
// Speculation — per-company bell-shape price events.
// ---------------------------------------------------------------------------
// Split out from the old merged "speculation_event.dart" (which used to
// cover both per-company hype AND speculation via a single shared
// CompanySpecEventType-branching mechanism). Hype has since been rebuilt
// as its own sector-wide mechanism (see ../hype/hype_event.dart) — this
// file is now speculation-only, in its own folder, per explicit
// instruction to keep each mechanism physically isolated so any one of
// them can be fixed without touching the others.
//
// Known bug, deferred by explicit instruction (its own future task):
// CompanySpecEvent.amplitude returns the bell curve's cumulative-so-far
// value, but _applySpecEvents below applies it as a raw per-tick
// multiplier every tick instead of a per-tick increment — the same
// un-dt-scaled compounding bug class already fixed for the macro epoch
// roll, News, and Hype. Not fixed here — this pass only separates
// speculation from hype, per the user's explicit "chinim speculation
// otdel'no" instruction.
// ---------------------------------------------------------------------------

extension SpeculationEventEngine on StressTestNotifier {
  // ── Per-Company Speculation Events ───────────────────────────────────
  // Each company gets one chance per weekly wall-clock window (see
  // lastSpecEventCheckAt in noise_engine.dart) to trigger a hidden
  // speculation event with bell-shape price impact (ramp up → reversal).
  // Cooldown: 3-4 weeks after event ends.
  // Events are evaluated in _simulateCurrentPrices.

  /// Try to fire a speculation event for a single holding. Called once per
  ...
  CompanySpecEvent? _maybeFireSpecEvent(
    ...
    // Peak amplitude: volatile ±5-15%, 50% positive / 50% negative.
    final peak =
  ...
  /// Apply active speculation events to price calculations.
